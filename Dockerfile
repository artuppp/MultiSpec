...
RUN /opt/conda/bin/pip install --no-cache-dir --no-deps basicsr 
RUN /opt/conda/bin/pip install --no-cache-dir --no-deps git+https://github.com/xinntao/Real-ESRGAN.git
...
import torch; \
model = SRVGGNetCompact(num_in_ch=3, num_out_ch=3, num_feat=64, num_block=8, act_type='relu', scale=4); \
...
print('Modelo Real-ESRGAN descargado y listo')" && \
    find /root -name "realesr-general-x4v3.pth" -exec cp {} /app/ \; 2>/dev/null || true
